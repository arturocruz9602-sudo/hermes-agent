...
class TelegramAdapter(BasePlatformAdapter):
    ...
    # 05 ago 2026 (fusion con upstream): sobreescritos a espanol -- la clase
    # base (gateway/platforms/base.py) trae estos en ingles por defecto, y el
    # mensaje de aprobacion de comandos es exactamente el flujo de botones
    # de F11-2 (USB) que Arturo ve en vivo. "Espanol siempre" es regla dura
    # del proyecto.
    _EA_HEADER = "⚠️ <b>Aprobación requerida</b>\n\n"
    _EA_CODE_OPEN = "<pre>"
    _EA_CODE_CLOSE = "</pre>\n\n"
    _EA_REASON_LABEL = "📌 <b>Motivo</b>: "
    ...
    async def send_exec_approval(
        ...
        try:
            # Nucleo compartido (header/comando/motivo/smart-deny) via la
            # plantilla de gateway/platforms/base.py -- atributos _EA_*
            # sobreescritos arriba a espanol. Las lineas extra (recursos
            # estimados, riesgo, hint de "Siempre") son propias de Telegram
            # y no forman parte del nucleo compartido, se agregan aparte.
            text = self._format_exec_approval(command, description, smart_denied)
            text += (
                f"\n⚡ <b>Recursos estimados</b>: ~200 tokens | RAM: 0 | Disco: 0\n"
                f"🔒 <b>Riesgo</b>: 🟡 Medio\n\n"
