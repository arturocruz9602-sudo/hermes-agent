...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        *, _persist_override: bool | None = None
                        ...
                        try:
                            ...
                            # NOTA (05 ago 2026, verificado antes de fusionar):
                            # upstream envuelve esto en asyncio.to_thread(), pero
                            # enrich_model_switch_warnings_for_gateway es `async
                            # def` -- pasarla a to_thread solo crea la corrutina
                            # en el hilo, nunca la ejecuta (jamas se hace
                            # awaited). Su propio test nuevo
                            # (test_model_command_context_offload.py) solo
                            # verifica que la funcion se DESPACHO a to_thread,
                            # no que su cuerpo corrio -- no habria detectado
                            # esto. Nos quedamos con el await directo, que si
                            # ejecuta el enriquecimiento de verdad.
        ...
        try:
            ...
            # Ver nota en el otro call site de esta funcion (mas arriba en
            # este archivo) sobre por que NO se envuelve en asyncio.to_thread.
            await enrich_model_switch_warnings_for_gateway(
                result,
                self,
                session_key=session_key,
