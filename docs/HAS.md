# HERMES ARCHITECTURE STANDARD (HAS) — ENTREGA COMPLETA · v1.6
...
**Cambios v1.6 (29-jul-2026 — triaje de propuestas externas de arquitectura, aprobado por Arturo tras revisar 3 documentos de análisis independiente):** contexto completo en `BLOQUES.md` ("Triaje de propuestas externas"). (1) **Nueva sección E13 "Recuperación total (la brecha real)"**: hoy no existe forma de reconstruir Hermes si la HP muere — el directorio pensado para esto (`/mnt/seagate/hermes_backups/`) está vacío desde su creación. (2) **B9 extendida**: el diario de reflexión pasa de semanal a nocturno, y se agrega un mecanismo de "reglas de comportamiento aprendidas" (siempre propuestas, nunca auto-adoptadas — mismo candado de aprobación que los hechos de memoria, después del incidente de contaminación de esta misma noche). (3) **Nueva sección E14 "Ventana de mantenimiento nocturna (2:00–5:00)"** (numerada aparte de E9, que ya usa "ventana de mantenimiento" para otro concepto — el ciclo de suscripción de Claude Pro; son dos ideas distintas con el mismo nombre, no confundir). (4) `GUION_PRUEBAS.md`: nueva regla de oro de crecimiento orgánico + casos R.8 (ráfaga de entradas simultáneas) y R.9 (reloj del sistema y suspensión). (5) Rechazado explícitamente por sobredimensión respecto a la escala real del proyecto (una laptop, un usuario, presupuesto de $100 MXN/mes): volumen de 2,400–4,000 pruebas formales y "100 pruebas por skill" (el principio de cobertura ya se cumple con la matriz combinatoria de `GUION_PRUEBAS.md` + la regla de crecimiento orgánico), diseñar cada decisión para un horizonte de 5 años o "500 skills" hipotéticas, y re-arquitecturas de mecanismos que ya funcionan (sistema de eventos, framework de skills, indexación de metadatos).
...
- **Traducción de la literatura a decisiones:** de **MemGPT/Letta** se toma la idea de memoria de trabajo editable (eso ya es `USER.md`, que la sección F3 convierte en consolidable); de **Generative Agents** se toma el *memory stream* con recencia+relevancia+importancia como fórmula de ranking del retrieval (implementable en 30 líneas sobre sqlite-vec); de **RAPTOR** se toma la idea de resúmenes jerárquicos — **diferida a Mac Mini** (generar el árbol de resúmenes cuesta LLM; en la HP el presupuesto no da); de **A-MEM/Zettelkasten** se toma que las notas enlazadas viven en **Obsidian**, no en una base paralela; **HippoRAG** (grafo de entidades) se descarta por complejidad/beneficio en un corpus de una sola persona. **Método experimental que sí vale la pena** (Arturo los pidió): "diario de reflexión" — Hermes (Gemini, gratis) escribe 5 observaciones de alto nivel sobre lo que hizo/qué falló/qué aprendió y las indexa como memorias de importancia alta; es la parte de "reflexión" de Generative Agents y es barata. **(v1.6, 29-jul):** pasa de cadencia semanal a **nocturna** (corre en la ventana de mantenimiento de E14) — el resumen semanal de domingo se conserva aparte, como el que de verdad lee Arturo; el nocturno es solo para que Hermes acumule su propia experiencia.
- **Reglas de comportamiento aprendidas (nueva en v1.6, 29-jul):** el barrido de memoria detecta patrones de Arturo repetidos **3 o más veces** (organización de archivos, horarios, formato preferido) y los propone como regla — nunca los adopta solo. Ejemplo: "he notado que siempre X, ¿lo hago automático?". Si Arturo aprueba, queda como hecho `tipo=regla` en `memoria_estructurada`, consultado por el prompt builder; mismo flujo de aprobación de un hecho normal (§OT-4). Nace directamente de la lección del incidente de esta noche: automatizar aprendizaje de reglas ANTES de tener auditoría y candado de aprobación sólido habría amplificado la contaminación encontrada, no evitarla.
...
## E13. Recuperación total (la brecha real) — nueva en v1.6

**Por qué existe:** verificado el 29-jul-2026 que `/mnt/seagate/hermes_backups/` — el directorio pensado para esto desde el 4 de julio — está y ha estado siempre vacío. Hoy, si la HP muere, lo único recuperable es el código (vive en GitHub vía el fork); memoria, configuración, skills e índices solo existen en ese disco. No es un escenario hipotético a 5 años: es un hueco real, hoy.

- **(a) `restaurar_hermes.sh`, versionado en el fork.** Desde una máquina Ubuntu limpia + el disco Seagate + el fork de GitHub, reconstruye: venv, config, memoria (`state.db`, `memoria_semantica.db`), skills, índices, timers systemd, servicios. Las llaves (`.env`, credenciales) se restauran desde una bóveda cifrada — la passphrase de Arturo es el único secreto no automatizable, a propósito.
- **(b) Prueba de restauración obligatoria.** La primera, al terminar de escribir el script (en un contenedor/VM o usuario limpio de la propia HP — nunca sobre el estado real en producción). Después, **cada 3 meses**. Un backup nunca probado no cuenta como backup — se marca así en la vista de avance hasta que se ejecute.
- **(c) `docs/RECUPERACION.md`.** El runbook humano: los pasos que Arturo (o alguien de su confianza) seguiría para reconstruir Hermes sin Claude Code disponible.

**Verificación E2E:** correr `restaurar_hermes.sh` sobre un entorno limpio reconstruye un Hermes funcional que responde por Telegram y recupera al menos un hecho real de memoria — sin ningún paso manual más allá de la passphrase de la bóveda.

## E14. Ventana de mantenimiento nocturna (2:00–5:00) — nueva en v1.6

**No confundir con E9 regla 7** ("ventana de mantenimiento" ahí significa el ciclo de suscripción de Claude Pro — un mes cada ~4 meses). Esta es la ventana operativa diaria: limpieza, reindexado, optimización de DB, diario de reflexión nocturno (B9) y backups corren **solo** entre las 2:00 y las 5:00. Si un mantenimiento invade horario de servicio (Hermes respondiéndole a Arturo con latencia degradada por una tarea de fondo), es bug, no característica.
...
- **L17** (la escalera de fallback dejaba la oferta de DeepSeek en silencio al caer todo lo gratis) — **RESUELTO Y GENERALIZADO (04 ago 2026, corrige el supuesto original de esta misma regla):** el diseño original (ago-jul) pedía un aviso "sí/no" antes de usar DeepSeek en caída total de la escalera gratuita. Quedó superado por el Bloque AN (30 jul): DeepSeek es `chat-primary`, el modelo principal, no un último recurso — cuando la escalera gratuita (`chat-gratis`) cae completa, el sistema cae a DeepSeek automáticamente, SIN preguntar, protegido por `max_turns=25`+`hard_stop`+el techo de $100 MXN/mes (no por un aviso caso por caso). Nunca calla el gasto (sigue en el ledger, reportado), pero ya no pide permiso — pedirlo generaba alarmas falsas de "posible violación" (hallazgo real 04 ago, watchdog P3 + reporte de Hermes) sobre un comportamiento que es, desde el 30 jul, el diseño correcto.
