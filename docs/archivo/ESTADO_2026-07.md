...
## Narrativa larga archivada desde ESTADO.md el 03 ago 2026
(compactada en ESTADO.md a una línea cada uno; el detalle completo también vive en docs/BLOQUES.md)
...
### F5-1 EN CURSO (02 ago) — vista "Finanzas" en Notion, tablero único (OT-5 Bloque 2)
`tools/notion_finanzas.py` (mismo patrón que `notion_avance_has.py`, B7: libreta.db fuente de verdad, Notion espejo solo-lectura): balance del mes, gastos por categoría, metas de ahorro, pagos recurrentes próximos. 7/7 pruebas en verde + verificado en vivo contra la API real (page_id `3b1c1df3-4107-8194-8baa-cab2ac2d44f0`). Timer `hermes-notion-finanzas.timer` activo cada 15 min. De las 6 vistas de OT-5 van 2/6 (Avance HAS, Finanzas). Faltan: Hoy, Kanban espejo, Cola de tareas, Escuela (bloqueada a Fase 6) + BLOQUE 1 (consolidar skill notion-api) + BLOQUE 3 (cola v2) + BLOQUE 4 (checklist diario).
Hallazgo sin arreglar: `notion_avance_has.py` sigue llamando a `has_progress.py` directo aunque reporta 90% engañoso.

### F5-2 CERRADO (02 ago) — cola de tareas v2 con garantía dura (OT-5 Bloque 3 / HAS §E5)
`scripts/cola_v2.py`: máquina de estados `encolada→en_proceso→resuelta→notificada|atorada` en `state.db`, escalera de reintentos Groq→Gemini→OpenRouter (5/prov, sin DeepSeek automático), watchdog de huérfanas >2h con idempotencia por result_hash, notificación GARANTIZADA, `task_queue_log` de éxito Y fallo. solver/notificador INYECTABLES. 11/11 pruebas. Pendiente de despliegue: cablear solver real + notificador real + timer systemd del watchdog cada 30 min.

### AT CERRADO (02 ago) — entrenador de trading testnet news-driven (OT-10/B4)
`scripts/trading_entrenador.py`: buy-the-dip informado por sentimiento (cruce caída+RSI<30+sentimiento alcista). Capital SIMULADO tope 5,000 MXN, ciclos semanales, freno duro -3% diario (r.40), fees 0.1%+slippage. Mercado y sentimiento INYECTABLES. Cliente `MercadoBinanceTestnet` cableado pero PENDIENTE. 17/17 pruebas.
Pendiente de despliegue (r.119): keys BINANCE_TESTNET_*, correr en Docker/QA, fuente de sentimiento real, reporte Notion.

### AU-1 CERRADO (02 ago) — motor de guiones desde Obsidian (Fase 8/OT-8, r.45-47)
`scripts/motor_guiones.py`: nota Obsidian → ANDAMIAJE de guion (gancho/promesa-payoff/segmentos con open-loops/cierre/CTA) + analizador de retención medible. Reglas ancladas en investigación web 02 ago. NO promete edición creativa. Registro en tabla `guiones` inyectable. 15/15 pruebas + verificado en vivo contra nota real del vault.
Pendiente: tool de runtime en el registry, pipeline de clips, OAuth YouTube.

### AU-2 CERRADO (02 ago) — pipeline de clips ≤2min + horarios + gate OAuth YouTube (Fase 8/OT-8/OT-11, r.46/48/59)
`scripts/pipeline_clips.py`: (1) extracción en ventanas ≤120s con tope DURO; (2) programación en mejores horarios (after-work 17:30-19:30 +23%, fin de semana +60%); (3) OAuth+publicación con PublicadorYouTube que se NIEGA a subir sin aprobado=True. 25/25 pruebas + CLI verificado.
Pendiente: OAuth real de Arturo, cliente YouTube Data API cableado, tool de runtime.

### AU-3 CERRADO (02 ago) — corte de silencios §E6 + edición DaVinci por SSH a la M1 (Fase 8/OT-8, r.102/r.49)
`scripts/corte_silencios.py`: corta silencios con verificación automática WER+palabras perdidas, resuelve el fallo real de r.49. `scripts/edicion_m1.py`: SesionM1 con triple candado (reversión en finally, ReversionError, auto-expiración caffeinate), EditorDaVinci sin render/efectos. 35/35 pruebas.
Pendiente de despliegue (r.119): ejecutor SSH real a la M1, Whisper real, calibración Yeti Nano.

### F6-1 CERRADO (03 ago) — correo personal: analiza→avisa→sugiere (Fase 6/OT-6, r.107-108)
`scripts/vigilar_correo_personal.py`: IMAP directo a Gmail personal, solo-lectura real, clasifica prioritario (banco/compras/eventos) vs ruido social. Blindaje "jamás la bandeja histórica" (4,245 correos). 13/13 pruebas + verificado en vivo (halló y corrigió 1 falso positivo real: "envio"/"envió"). DESPLEGADO: timer cada 15 min.
