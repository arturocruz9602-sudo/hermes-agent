...
- ✅ **AS-2 Cierre nocturno en AUDIO (r.90) — generación+verificación HECHA, envío en vivo PENDIENTE:** `scripts/cierre_del_dia_audio.py` arma el resumen (saldo/gastos/pagos/tareas, solo lectura de la libreta) y lo pasa a voz reusando `tools.tts_tool.text_to_speech_tool` (NO se creó TTS nuevo). Probado en simulación 02 ago: `--generar --forzar` → `cierre_20260802_172054.ogg` (74,627 bytes) verificado con `ffprobe` = codec opus real. `~/.hermes/scripts/enviar.py:detectar_tipo()` ahora reconoce `.ogg`→`sendVoice` (nota de voz nativa); verificado SIN tráfico real (`requests.post` mockeado, confirmó URL `/sendVoice` + field `voice`). Candado r.119 sigue: `.env` solo tiene `TELEGRAM_HOME_CHANNEL` real, no hay canal QA — el envío en vivo (real o QA) NO se disparó y queda pendiente de que exista canal QA o Arturo apruebe mandarlo a producción. Timer nocturno (`hermes-cierre-dia-audio.timer`) NO armado todavía — falta que Arturo confirme la hora (r.90 sigue sin "ok" formal, default 22:45).
- ⚠️ Hallazgo (fuera de alcance de AS-2, anotado no arreglado): `~/.hermes/scripts/enviar.py` vive SOLO en `~/.hermes/` — no tiene copia en este repo ni versión (a diferencia de `cierre_del_dia.py`, que sí es idéntico en ambos lados). Cualquier edición ahí (como la de hoy) no queda en git.
