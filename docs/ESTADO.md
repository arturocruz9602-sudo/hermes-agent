...
## Último contexto
02 ago: el loop despachó AU-1 (motor de guiones) y AU-2 (pipeline de clips ≤2min + horarios + gate OAuth YouTube), además de F5-1/F5-2/AT esta sesión. Cadena de YouTube (r.97 #2) ya cubre: idea→guion→clips→horarios→gate de subida; todo espera SOLO el OAuth de 5 min de Arturo. Siguiente: orquestador multi-modelo, tool de runtime AU en el registry, retomar AS, OT-5 (Hoy/Kanban/Escuela + Bloques 1/4); F5-2/AT/AU pendientes solo de despliegue.
