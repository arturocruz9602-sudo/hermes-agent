...
async def test_warning_enrichment_is_offloaded(tmp_path, monkeypatch):
    ...
    via merge_preflight_compression_warning, so the blocking call must be
    dispatched through asyncio.to_thread rather than running inline on the loop.

    05 ago 2026 (fusion con upstream): enrich_model_switch_warnings_for_gateway
    en si es `async def` -- pasarla completa a asyncio.to_thread() (lo que
    hacia la version original de esta prueba/commit) solo crea la corrutina
    en el hilo, sin nunca hacerle await, asi que el enriquecimiento real
    JAMAS corria. El offload correcto vive un nivel mas adentro: dentro de
    esa funcion async, sobre la llamada sincrona real
    (merge_preflight_compression_warning) -- ver
    hermes_cli/context_switch_guard.py. Esta prueba ahora verifica ESA
    llamada, que es la que de verdad bloqueaba el loop."""
    ...
    runner = _runner_with_store(tmp_path, monkeypatch)
    # A cached agent is required for enrich_model_switch_warnings_for_gateway
    # to reach merge_preflight_compression_warning at all -- without one it
    # returns early (agent is None) and neither the old buggy to_thread-on-
    # async-def call nor a correct implementation would ever dispatch
    # anything real, making the assertion below meaningless either way.
    fake_agent = MagicMock()
    fake_agent.compression_enabled = True
    fake_agent.context_compressor = MagicMock(context_length=128000)
    fake_agent._custom_providers = None
    session_key = runner._session_key_for_source(_make_source())
    runner._agent_cache_lock = threading.Lock()
    runner._agent_cache = {session_key: (fake_agent, None, None)}

    result = await runner._handle_model_command(_event("/model gpt-5.5"))

    assert result is not None and "gpt-5.5" in result
    assert context_switch_guard.merge_preflight_compression_warning in offloaded, (
        "merge_preflight_compression_warning (the actual blocking sync call) "
        "must be dispatched via asyncio.to_thread (it was called inline on "
