...
def finalize_turn(
    ...
    *,
    ...
    try:
        ...
        #
        # Placed HERE (Bloque AF's post-correction persist point), not at
        # upstream's earlier position, for the same reason persist itself is
        # here: T.6's secret scanner and the anti-fabrication backstop run
        # AFTER this block and can still rewrite `final_response`/`messages[-1]`
        # (see the Bloque AF comment above). Compacting before those passes
        # would let an unredacted secret or a fabricated claim get folded into
        # the rolling summary before it's ever caught.
        if not interrupted and not failed:
            try:
                ...
                if (
                    ...
                    # dict in place. The compressor has no agent reference,
                    # so it raises a flag for us to invalidate the bounded
                    # flush-scan cursor; otherwise the rewritten marker row
                    # is identity-skipped and the stale summary persists to
                    # state.db.
                    if getattr(
