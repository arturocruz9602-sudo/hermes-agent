...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "QUERY QUALITY (Bloque U, 23 Jul 2026 -- real failure found and fixed)\n\n"
        "  FTS5 is lexical (BM25 keyword ranking), not semantic -- it only finds what "
        "shares actual words with the query. A generic restatement of the user's "
        "question (e.g. query=\"Hermes project\" for \"en qué se quedó el proyecto de "
        "Hermes\") loses almost all of the discriminating power and can surface an "
        "unrelated session instead of the right one -- verified: the SAME real question, "
        "searched with query=\"proyecto Hermes estado\" or the user's own words "
        "verbatim, correctly found the right session; query=\"Hermes project\" did not.\n"
        "  - Use SPECIFIC keywords, not a generic restatement -- pull concrete nouns "
        "from the user's own message (names, features, error strings, file/service "
        "names) rather than paraphrasing the question shape.\n"
        "  - Match the LANGUAGE of the conversation. Do not silently translate the "
        "query to English when the conversation is in Spanish (or vice versa) -- the "
        "indexed content is in whatever language it was actually written in.\n"
        "  - If a first discovery call comes back weak/irrelevant, do not just tell the "
        "user it's not found -- retry ONCE with different, more specific terms (e.g. "
        "from something concrete the user just said) before concluding there's nothing "
        "there. Only ask the user for a keyword after that retry still comes up empty."
    ),
    "parameters": {
        "type": "object",
        "properties": {
